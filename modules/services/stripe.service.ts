...
export const stripeService = {
    webhook: async function (requestBuffer: string, stripeSignature: string) {
        ...
        switch (event.type) {
            case 'customer.subscription.created': {
                await this.subscription.create(event.data.object);
                return;
            }

            case 'customer.subscription.updated': {
                await this.subscription.update(event.data.object);
                return;
            }

            case 'invoice.paid': {
                await this.subscription.updateInvoicePeriod(event.data.object);
                return;
            }
        }
    },
    ...
    subscription: {
        create: async function (subscription: Stripe.Subscription) {
            // get business id
        ...
        update: async function (subscription: Stripe.Subscription) {
            // get business id
        ...
        updateInvoicePeriod: async function (invoice: Stripe.Invoice) {
            // get business id
