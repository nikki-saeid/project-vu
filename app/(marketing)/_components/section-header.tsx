...
export default function SectionHeader({ label, title, description, center = true, isWhite = false }: SectionHeaderProps) {
    return (
        <div className={cn('flex flex-col', center && 'items-center')}>
            <Badge className={cn('mb-4 font-medium text-xs', isWhite ? 'bg-white/30 text-white' : 'bg-primary/5 text-primary')}>
                {label}
            </Badge>
            {title && (
                <H2 className={cn('text-2xl', description && 'mb-4', center && 'text-center', isWhite ? 'text-white' : 'text-foreground')}>
                    {title}
                </H2>
            )}
            {description && (
